...
// const rimBackboardOffset = 0.04;
//
// const leftRimCenter = new THREE.Vector3(
//     -courtLength / 2 + backboardThickness + rimRadius,
//     rimHeight,
//     0
// );
// const rightRimCenter = new THREE.Vector3(
//     courtLength / 2 - backboardThickness - rimRadius,
//     rimHeight,
//     0
// );
...
//   const baselineX = side * (courtLength / 2);
//
//   // Backboard is ON the baseline, facing court
//   const backboardX = baselineX + side * (backboardThickness / 2);
//   const rimX = baselineX + side * (backboardThickness + rimRadius);
//
//   // --- Backboard mesh ---
//   const backboard = new THREE.Mesh(
//       new THREE.BoxGeometry(backboardThickness, backboardHeight, backboardWidth),
//       new THREE.MeshPhongMaterial({ color: 0xffffff, opacity: 0.85, transparent: true })
//   );
//   backboard.position.set(backboardX, rimHeight + backboardHeight / 2 - 0.15, 0);
//   backboard.castShadow = true;
//   backboard.receiveShadow = true;
//   scene.add(backboard);
//
...
//   rim.rotation.y = side === -1 ? Math.PI : 0;
//   rim.castShadow = true;
//   rim.receiveShadow = true;
//   scene.add(rim);
//
...
//   if (attemptsEl) attemptsEl.innerText = shotAttempts;
//   if (madeEl) madeEl.innerText = shotsMade;
//   if (pctEl) {
//     const pct = shotAttempts > 0 ? ((shotsMade / shotAttempts) * 100).toFixed(1) : "0";
//     pctEl.innerText = pct;
//   }
...
// }
//
// // IMPROVED: Better shot velocity calculation
// function getShotVelocityFromPower(distance) {
//   // Base velocity that scales with distance and power
//   const baseVelocity = 6.0 + (distance * 0.4); // Slightly lower base, better scaling
//   const powerMultiplier = 0.7 + (shotPower / 100) * 0.8; // Better power curve
//   return baseVelocity * powerMultiplier;
// }
//
// function shootBall() {
...
//
//   // Debug: Log shot info
//   console.log(`Shot Info: Distance=${distanceXZ.toFixed(2)}m, Power=${shotPower}%, Velocity=${vxz.toFixed(2)}m/s`);
//   console.log(`Target: (${targetHoop.x.toFixed(2)}, ${targetHoop.y.toFixed(2)}, ${targetHoop.z.toFixed(2)})`);
//   console.log(`Ball: (${basketball.position.x.toFixed(2)}, ${basketball.position.y.toFixed(2)}, ${basketball.position.z.toFixed(2)})`);
// }
//
// // IMPROVED: Better score detection
// function checkForScore() {
//   if (!lastTargetHoop || shotJustScored) return;
//
//   const ballXZ = new THREE.Vector2(basketball.position.x, basketball.position.z);
...
//   const wasAbove = lastBallY !== null && lastBallY > lastTargetHoop.y;
//   const nowBelow = basketball.position.y <= lastTargetHoop.y;
//   const movingDown = ballVelocity.y < 0;
//   const withinRim = distXZ < rimRadius * 0.75; // Slightly smaller for cleaner shots
//
//   if (wasAbove && nowBelow && movingDown && withinRim) {
//     // Successful shot!
//     score += 2;
//     shotsMade += 1;
//     shotJustScored = true;
//     showShotResultDisplay("SHOT MADE!", "lime");
//     updateScoreUI();
//     console.log("SHOT MADE! Distance from rim:", distXZ.toFixed(3));
//   }
// }
//
// // Handle key events
...
//       lastBallY = basketball.position.y;
//
//       // IMPROVED: Better rim collision detection
//       [leftRimCenter, rightRimCenter].forEach(rimCenter => {
//         const ballXZ = new THREE.Vector2(basketball.position.x, basketball.position.z);
//         const rimXZ = new THREE.Vector2(rimCenter.x, rimCenter.z);
//         const distXZ = ballXZ.distanceTo(rimXZ);
//
//         if (distXZ < rimRadius + ballRadius) {
//           // Allow clean shots to pass through
//           const isCleanShot = basketball.position.y > rimCenter.y + 0.1 &&
//               ballVelocity.y < -1.0 &&
//               distXZ < rimRadius * 0.75;
//
//           if (!isCleanShot) {
//             // Collision with rim
//             const overlap = (rimRadius + ballRadius) - distXZ;
...
//         if (worldMove.length() > 0.01) {
//           const axis = new THREE.Vector3().crossVectors(worldMove, new THREE.Vector3(0, 1, 0)).normalize();
//           const angularSpeed = worldMove.length() / ballRadius;
//           basketball.rotateOnAxis(axis, angularSpeed * dt);
//         }
//       }
//
